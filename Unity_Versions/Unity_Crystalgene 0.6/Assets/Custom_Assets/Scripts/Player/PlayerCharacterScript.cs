...
{    
    ...
    private void ControllerMovement()
    {
        ...
        if(!controller.isGrounded)
        {
            Vector3 airChange = (transform.right * horizontal + transform.forward * vertical).normalized * 13f * Time.unscaledDeltaTime;
            lastMotion += airChange;

            controllerVelocity.x = Mathf.Clamp(lastMotion.x, -movementSpeed, movementSpeed);
            controllerVelocity.z = Mathf.Clamp(lastMotion.z, -movementSpeed, movementSpeed);
        }
