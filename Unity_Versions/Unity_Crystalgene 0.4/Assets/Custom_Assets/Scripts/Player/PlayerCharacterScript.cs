...
{    
    ...
    {
        playerCamera.GetComponent<Animator>().Play("Headbob");
    }

    private void StopBobbing()
    {
        if (playerCamera.GetComponent<Animator>().GetCurrentAnimatorStateInfo(0).normalizedTime >1)
        {
        playerCamera.GetComponent<Animator>().Play("Default");
        }
    }    
}
